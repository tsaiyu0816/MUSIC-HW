#!/usr/bin/env python
# coding: utf-8
# generate.py — 支援 custom(.pkl) 與 miditok（無需 .pkl）兩種生成

import argparse, os, sys, pickle, random, shutil, subprocess, re
from pathlib import Path
from typing import Dict, List, Tuple, Optional

import numpy as np
import torch
from transformers import GPT2LMHeadModel, GPT2Config

# ----- path shim for utils.py -----
HERE = Path(__file__).resolve().parent
for cand in (HERE, HERE.parent, HERE.parent.parent):
    if (cand / "utils.py").exists():
        sys.path.insert(0, str(cand))
        break
import utils  # noqa: E402

# ===== Optional miditok =====
HAVE_MIDITOK = False
try:
    from miditok import TokenizerConfig, REMI
    try:
        from miditok import TokSequence  # >=3.x
    except Exception:
        from miditok.utils import TokSequence
    HAVE_MIDITOK = True
except Exception:
    HAVE_MIDITOK = False

X_LEN = 1024  # context length


# ---------- helpers ----------
def render_midi_to_wav(midi_path: str, wav_path: str, sf2_path: str, sr: int = 22050) -> bool:
    """優先 fluidsynth CLI；不行再用 midi2audio.FluidSynth。成功回傳 True。"""
    if shutil.which("fluidsynth"):
        cmd = ["fluidsynth", "-ni", "-T", "wav", "-F", str(wav_path), "-r", str(sr), sf2_path, midi_path]
        try:
            subprocess.run(cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            return True
        except subprocess.CalledProcessError:
            pass
    try:
        from midi2audio import FluidSynth  # lazy import
        FluidSynth(sound_font=sf2_path, sample_rate=sr).midi_to_audio(midi_path, wav_path)
        return True
    except Exception:
        return False


def temp_topk_sample(logits_np, temperature=1.0, top_k=0) -> int:
    z = logits_np.astype(np.float64) / max(1e-8, float(temperature))
    if top_k and 0 < top_k < z.shape[0]:
        idx = np.argpartition(-z, top_k)[:top_k]
        vals = z[idx]
        p = np.exp(vals - vals.max()); p /= p.sum()
        return int(np.random.choice(idx, p=p))
    z -= z.max(); p = np.exp(z); p /= p.sum()
    return int(np.random.choice(len(z), p=p))


def load_dictionary(dict_path: Path) -> Tuple[Dict[str, int], Dict[int, str]]:
    """相容 (e2w, w2e) 或 (w2e, e2w) 或單一 dict 格式。"""
    obj = pickle.load(open(dict_path, "rb"))
    if isinstance(obj, tuple) and len(obj) == 2:
        a, b = obj
        if isinstance(a, dict) and isinstance(b, dict):
            if all(isinstance(k, str) for k in a.keys()):
                e2w, w2e = a, b
            else:
                e2w, w2e = b, a
            return e2w, w2e
    if isinstance(obj, dict):
        if all(isinstance(k, str) for k in obj.keys()):
            e2w = obj
            w2e = {v: k for k, v in e2w.items()}
            return e2w, w2e
        if all(isinstance(k, int) for k in obj.keys()):
            w2e = obj
            e2w = {v: k for k, v in w2e.items()}
            return e2w, w2e
    raise SystemExit(f"[generate] 不支援的 dictionary.pkl 格式：{dict_path}")


def _extract_state_dict(sd_raw):
    # 取出 state_dict 並移除常見前綴
    sd = sd_raw
    if isinstance(sd_raw, dict):
        for k in ("model", "state_dict", "weights", "params"):
            if k in sd_raw and isinstance(sd_raw[k], dict):
                sd = sd_raw[k]
                break
    if not isinstance(sd, dict):
        raise SystemExit("[generate] checkpoint 不是可讀的 state_dict")
    cleaned = {}
    for k, v in sd.items():
        nk = k
        for pref in ("module.", "model.", "gpt.", "gpt2."):
            if nk.startswith(pref):
                nk = nk[len(pref):]
        cleaned[nk] = v
    return cleaned


def build_model(vocab_size: int, ckpt_path: Path, device: str) -> GPT2LMHeadModel:
    sd_raw = torch.load(ckpt_path, map_location="cpu")
    cfg_dict = {}
    if isinstance(sd_raw, dict):
        for k in ("config", "gpt_cfg", "cfg"):
            if k in sd_raw and isinstance(sd_raw[k], dict):
                cfg_dict = dict(sd_raw[k])
                break
    cfg_dict.setdefault("vocab_size", vocab_size)
    cfg_dict.setdefault("n_positions", X_LEN)
    cfg_dict.setdefault("n_ctx", X_LEN)
    cfg = GPT2Config(**cfg_dict)
    state_dict = _extract_state_dict(sd_raw)
    model = GPT2LMHeadModel(cfg)
    missing, unexpected = model.load_state_dict(state_dict, strict=False)
    if missing:
        print(f"[warn] missing keys: {len(missing)} -> {missing[:5]}")
    if unexpected:
        print(f"[warn] unexpected keys: {len(unexpected)} -> {unexpected[:5]}")
    model.to(device).eval()
    return model


# ---------- custom tokenizer path ----------
def init_seq_from_prompt_custom(e2w: Dict[str, int], prompt_midi: Optional[str]) -> List[int]:
    if prompt_midi:
        note_items, tempo_items = utils.read_items(prompt_midi)
        note_items = utils.quantize_items(note_items)
        max_time = int(note_items[-1].end) if len(note_items) > 0 else 0
        groups = utils.group_items(tempo_items + note_items, max_time)
        events = utils.item2event(groups)
        keys = [f"{ev.name}_{ev.value}" for ev in events][:X_LEN]
        seq = [e2w[k] for k in keys if k in e2w]
        if seq:
            return seq
    seq = []
    if "Bar_None" in e2w:
        seq.append(e2w["Bar_None"])
    pos = e2w.get("Position_1/16", None)
    if pos is not None:
        seq.append(pos)
    tcls = [v for k, v in e2w.items() if k.startswith("Tempo Class_")]
    tval = [v for k, v in e2w.items() if k.startswith("Tempo Value_")]
    if tcls: seq.append(int(np.random.choice(tcls)))
    if tval: seq.append(int(np.random.choice(tval)))
    return seq


def generate_one_custom(model, start_tokens, bars, e2w, temperature, top_k, device, max_factor=2048):
    out_tokens = list(start_tokens)
    bars_done = 0
    max_steps = max(X_LEN, bars * max_factor)
    with torch.inference_mode():
        for _ in range(max_steps):
            x = torch.tensor(out_tokens[-X_LEN:], dtype=torch.long, device=device)[None, :]
            logits = model(x).logits[0, -1].detach().cpu().numpy()
            nxt = temp_topk_sample(logits, temperature=temperature, top_k=top_k)
            out_tokens.append(int(nxt))
            if nxt == e2w.get("Bar_None", -1):
                bars_done += 1
                if bars_done >= bars:
                    break
    return out_tokens


def save_midi_custom(e2w: Dict[str, int], tokens: List[int], out_mid: Path):
    word2event = {i: k for k, i in e2w.items()}
    utils.write_midi(words=tokens, word2event=word2event, output_path=str(out_mid), prompt_path=None)


# ---------- miditok path ----------
def _safe_ids_to_tokens(tokenizer, ids):
    try:
        return tokenizer.ids_to_tokens(ids, as_str=True)
    except Exception:
        toks = []
        for i in ids:
            try:
                toks.append(str(tokenizer[i]))
            except Exception:
                toks.append(None)
        return toks


def build_miditok_config(kind: str, with_chord: bool) -> TokenizerConfig:
    # 與訓練一致：1/16、32 velocities、開 rests/tempos；REMI+ 再開 program / time_sig
    return TokenizerConfig(
        beat_res={(0, 4): 4},
        num_velocities=32,
        use_chords=bool(with_chord),
        use_rests=False,
        use_tempos=True,
        use_time_signatures=(kind == "remi_plus"),
        use_programs=(kind == "remi_plus"),
        one_token_stream_for_programs=(kind == "remi_plus"),
    )


def build_bar_checker(tokenizer):
    bar_ids = set()
    try:
        for i, t in enumerate(_safe_ids_to_tokens(tokenizer, list(range(len(tokenizer))))):
            if isinstance(t, str):
                head = t.split("_", 1)[0].lower()
                if head == "bar" or "bar" in head:
                    bar_ids.add(i)
    except Exception:
        pass

    def is_bar(tok_id: int) -> bool:
        if tok_id in bar_ids:
            return True
        t = _safe_ids_to_tokens(tokenizer, [tok_id])[0]
        if isinstance(t, str):
            head = t.split("_", 1)[0].lower()
            return (head == "bar" or "bar" in head)
        return False

    return is_bar


def encode_prompt_miditok(tokenizer: REMI, prompt_midi: Optional[str]) -> List[int]:
    if not prompt_midi:
        return []
    try:
        from symusic import Score
        try:
            seq = tokenizer.encode(Score(prompt_midi))
        except Exception:
            seq = tokenizer.midi_to_tokens(Score(prompt_midi))
        if isinstance(seq, list):
            assert len(seq) == 1
            seq = seq[0]
        return list(seq.ids[:X_LEN])
    except Exception:
        return []


def generate_one_miditok(model, tokenizer: REMI, start_ids, bars, temperature, top_k, device):
    is_bar = build_bar_checker(tokenizer)
    out_ids = list(start_ids)
    bars_done = 0
    hard_cap = bars * X_LEN * 2
    with torch.inference_mode():
        while bars_done < bars and len(out_ids) < hard_cap:
            x_np = np.array(out_ids[-X_LEN:], dtype=np.int64)[None, :] if len(out_ids) else np.zeros((1, 1), dtype=np.int64)
            x = torch.from_numpy(x_np).long().to(device)
            logits = model(x).logits[0, -1].detach().cpu().numpy()
            nxt = temp_topk_sample(logits, temperature=temperature, top_k=top_k)
            out_ids.append(int(nxt))
            if is_bar(int(nxt)):
                bars_done += 1
    return out_ids


def save_midi_miditok(tokenizer: REMI, ids: List[int], out_mid: Path):
    seq_obj = TokSequence(ids=ids)
    tokenizer.complete_sequence(seq_obj)
    try:
        midi_obj = tokenizer.decode(seq_obj)
        if hasattr(midi_obj, "save_midi"):
            midi_obj.save_midi(str(out_mid))
        elif hasattr(midi_obj, "dump_midi"):
            midi_obj.dump_midi(str(out_mid))
        elif hasattr(midi_obj, "to_pretty_midi"):
            pm = midi_obj.to_pretty_midi(); pm.write(str(out_mid))
        else:
            pm = tokenizer.tokens_to_midi(seq_obj); pm.write(str(out_mid))
    except Exception:
        pm = tokenizer.tokens_to_midi(seq_obj); pm.write(str(out_mid))


# ---------- main ----------
def main():
    ap = argparse.ArgumentParser("Batch MIDI/WAV generator (custom or miditok)")
    ap.add_argument("--ckpt", required=True, help="模型 checkpoint 檔（.pt / .pkl）")
    ap.add_argument("--tokenizer", choices=["custom", "remi", "remi_plus"], default="custom",
                    help="custom 需 .pkl；remi / remi_plus 為 miditok（不需 .pkl）")
    ap.add_argument("--dict_path", default="", help="(custom only) dictionary.pkl（e2w/w2e）")
    ap.add_argument("--with_chord", type=int, default=0, help="miditok/custom 均可傳（custom 由字典決定是否有 Chord 事件）")
    ap.add_argument("--out_dir", required=True, help="輸出資料夾")
    ap.add_argument("--num", type=int, default=20, help="生成幾首")
    ap.add_argument("--bars", type=int, default=32, help="目標 bars")
    ap.add_argument("--temperature", type=float, default=1.0)
    ap.add_argument("--top_k", type=int, default=0)
    ap.add_argument("--seed", type=int, default=3407)
    ap.add_argument("--prompt_midi", default="", help="以此 MIDI 當起手（可省略）")
    ap.add_argument("--sf2", type=str, default="./assets/sf2/GM.sf2", help="SoundFont 路徑（渲染 WAV 用）")
    ap.add_argument("--sr", type=int, default=22050, help="WAV 取樣率")
    ap.add_argument("--prefix", type=str, default="sample", help="輸出檔名前綴")
    args = ap.parse_args()

    out_dir = Path(args.out_dir); out_dir.mkdir(parents=True, exist_ok=True)
    np.random.seed(args.seed); random.seed(args.seed); torch.manual_seed(args.seed)
    device = "cuda" if torch.cuda.is_available() else "cpu"

    # --- tokenizer & vocab ---
    if args.tokenizer == "custom":
        if not args.dict_path:
            raise SystemExit("[generate] custom 模式需要 --dict_path 指向 dictionary.pkl")
        e2w, w2e = load_dictionary(Path(args.dict_path))
        vocab_size = max(e2w.values()) + 1
        tokenizer = None
    else:
        if not HAVE_MIDITOK:
            raise SystemExit("[generate] 需要 pip install miditok symusic")
        kind = "remi_plus" if args.tokenizer == "remi_plus" else "remi"
        cfg = build_miditok_config(kind, bool(args.with_chord))
        tokenizer = REMI(cfg)
        vocab_size = len(tokenizer)
        e2w = w2e = None

    # --- model ---
    model = build_model(vocab_size=vocab_size, ckpt_path=Path(args.ckpt), device=device)

    total_ok = 0
    for i in range(args.num):
        seed_i = args.seed + i
        np.random.seed(seed_i); random.seed(seed_i); torch.manual_seed(seed_i)

        if args.tokenizer == "custom":
            start_tokens = init_seq_from_prompt_custom(e2w, args.prompt_midi or None)
            tokens = generate_one_custom(
                model=model, start_tokens=start_tokens, bars=args.bars,
                e2w=e2w, temperature=args.temperature, top_k=args.top_k, device=device
            )
            out_mid = out_dir / f"{args.prefix}_{i:04d}.mid"
            try:
                save_midi_custom(e2w, tokens, out_mid)
                print(f"[gen] wrote {out_mid}")
            except Exception as e:
                print(f"[gen] 寫入 MIDI 失敗：{out_mid} — {e}")
                continue
        else:
            # miditok path
            start_ids = encode_prompt_miditok(tokenizer, args.prompt_midi or None)
            ids = generate_one_miditok(
                model=model, tokenizer=tokenizer, start_ids=start_ids, bars=args.bars,
                temperature=args.temperature, top_k=args.top_k, device=device
            )
            out_mid = out_dir / f"{args.prefix}_{i:04d}.mid"
            try:
                save_midi_miditok(tokenizer, ids, out_mid)
                print(f"[gen] wrote {out_mid}")
            except Exception as e:
                print(f"[gen] 寫入 MIDI 失敗：{out_mid} — {e}")
                continue

        # render wav
        if os.path.isfile(args.sf2):
            out_wav = out_mid.with_suffix(".wav")
            ok = render_midi_to_wav(str(out_mid), str(out_wav), args.sf2, sr=args.sr)
            print(f"[gen] rendered {out_wav}" if ok else f"[gen] render failed → {out_wav}")
        else:
            print(f"[gen] skip render: sf2 not found → {args.sf2}")

        total_ok += 1

    print(f"[done] 成功輸出 {total_ok}/{args.num} 首到 {out_dir}")


if __name__ == "__main__":
    main()
